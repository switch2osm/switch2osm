...
permalink: /serving-tiles/manually-building-a-tile-server-debian-12/
---

This page describes how to install, setup and configure all the necessary software to operate your own tile server. These step-by-step instructions were written for [Debian Linux](https://www.debian.org/) 12 stable (bookworm), and were tested in July 2023.
...
These instructions are have been written and tested against a newly-installed Debian 12 server. If you have got other versions of some software already installed (perhaps you upgraded from an earlier version, or you set up some PPAs to load from) then you may need to make some adjustments.
...
The version of Mapnik provided by Debian 12 at release was 3.1, and therefore the “plugins_dir” setting in the “[mapnik]” part of this file is “/usr/lib/mapnik/3.1/input”.  This may change in the future. If an error occurs when trying to render tiles such as this:
...
If you look at the system log, you should see messages from the “renderd” service.  Note that on Debian 12, messages are no longer written to '/var/log/syslog' by default - to follow messages as they are written, do this: 

    sudo journalctl -ef

See [here](https://www.debian.org/releases/bookworm/amd64/release-notes/ch-information.en.html#changes-to-system-logging) for more details.

    sudo /etc/init.d/apache2 restart
    
Using "sudo journalctl -ef" you should see a line such as:
