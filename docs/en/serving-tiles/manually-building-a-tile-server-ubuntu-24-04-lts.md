...
Thanks to the work done by the Debian and Ubuntu maintainers to incorporate the latest versions of these packages into {{ dist }}, these instructions are somewhat shorter than earlier versions.
...
When this guide was first written, the version of Mapnik provided by {{ dist }} was 3.1, and the `plugins_dir` setting in the `[mapnik]` part of the file was `/usr/lib/mapnik/3.1/input`. That "3.1" may change again in the future. If an error occurs when trying to render tiles such as this:
